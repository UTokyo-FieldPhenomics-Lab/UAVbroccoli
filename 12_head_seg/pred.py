...
def predict_batch(model, batch_images, device='cpu'):
    """[summary]
...
def pred_whole_image(img_path, coords, model, base=75, transform=None):
    ...
    for box, mask in zip(bboxs, masks):
        ...
        # print(mask.shape)
        whole_mask[y0:y1, x0:x1, :] = whole_mask[y0:y1, x0:x1, :] + mask
    # iio.imsave('./test.png', whole_mask)
    # np.save('whole_mask.npy', whole_mask)
    whole_mask[whole_mask > 200] = 255
    return mask2polygon(whole_mask[:, :, 0]), whole_mask
